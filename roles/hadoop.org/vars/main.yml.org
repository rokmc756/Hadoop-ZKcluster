...
netinfo:
  ipaddr:
    master: "{{ hostvars[groups['master'][0]]['ansible_'~ netdev0]['ipv4']['address'] }}"
    standby: "{{ hostvars[groups['standby'][0]]['ansible_'~ netdev0]['ipv4']['address'] }}"
  hostname:
...
# hadoop_path: "/home/hadoop" --> hadoop.base_path
...
# hadoop_config_path: "{{ hadoop.base_path }}/hadoop-{{ hadoop_version }}/etc/hadoop"
# hadoop_tmp: "{{ hadoop.base_path }}/tmp"
# hadoop_dfs_name: "{{ hadoop.base_path }}/dfs/name"
# hadoop_dfs_data: "{{ hadoop.base_path }}/dfs/data"
# hadoop_log_path: "{{ hadoop.base_path }}/hadoop_logs"
# hadoop_journalnode_path: "{{ hadoop.base_path }}/journalnode/data"
...
list:
  zkservers: "{{ groups['all'] | map('extract', hostvars, ['ansible_fqdn']) | map('regex_replace', '$', ':2181') | join(',') }}"
  qjournal: "{{ groups['all'] | map('extract', hostvars, ['ansible_hostname']) | map('regex_replace', '$', '.jtest.pivotal.io:8485') | join(';') }}"
...
hdfs_site_properties:
  ...
  - {
      ...
      "value":"qjournal://{{ list.qjournal }}/{{ hdfs.df.nameservices }}"
  }
  - {
