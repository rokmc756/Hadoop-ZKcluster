...
for i in `echo "$HOSTS"`
do
    ssh root@$i "rm -rf /home/hadoop/* /home/hadoop/.ssh/* /tmp/*hadoop-hadoop-*.pid /usr/local/apache-zookeeper* /usr/local/kafka* /var/lib/zookeeper /var/log/zookeeper"
    ssh root@$i "systemctl stop zookeeper"
    ssh root@$i "killall java"
    ssh root@$i "rm -rf /var/lib/zookeeper /var/log/zookeeper"
